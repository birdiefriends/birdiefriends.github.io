...
export default {
  async fetch(request, env) {
    ...
    // Body: { event_name, player, holes: [18 numbers|null], marks?: [18 strings|null],
    //         tee_box?: string, front9?, back9?, total? }
    ...
    // tee_box is a single value from a fixed client-side chip list (Front/
    // Middle/Back/Combo + common color names) — never freeform, since course
    // tee naming isn't standardized and freeform text can't be aggregated
    // reliably later. Holes may be all-null for a "quick" total-only entry
    // (front9/back9/total set directly) — no separate flag needed, a fully
    // null holes array with a populated total is a valid, meaningful row.
