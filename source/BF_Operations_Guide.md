...
|-----------|---------|--------|
| Portal | v3.17.133 · 2026-08-11 | Production ✅ — Dev-70: BSGC/BSCG venue-typo scorecard fix + "See Official Results" banner scoped to real BF Series events only; Auto-Repeat Gatherings (checkbox on Create/Edit, crew-mode only, spawns successor a week after tee time passes, chains until cancelled/unchecked); Gathering capacity enforcement (overflow Yes auto-downgrades to Sub, notifies host) |
| GolfScorer | v8.57 · 2026-08-11 | Deployed ✅ — Dev-70: Groups ranking now matches the real Series 4-event qualification split (was pooling everyone by score regardless of event count); unassigned pool always rank-sorted on render; local Jotform proxy calls retry with backoff (proxy-not-ready-at-launch race); HCP-override protection across all three write paths (manual edit ×2, GHIN copy/paste); NoHCP↔real-HCP tee-cell dropdown fixed both directions, HCP min=0 floor removed |
| Worker | 2026-08-11 (Dev-70) | Deployed ✅ — Auto-Repeat Gatherings (`spawnAutoRepeatGatherings()`, fired via `ctx.waitUntil()` on every `GET /gatherings`), Gathering capacity enforcement in `POST /registrations`. Temporary `/admin/fix-venue-typo` route used for a 160-row Jotform bulk fix, then removed. Manually pasted into Cloudflare dashboard at each stage and confirmed live by Brian. |
...
| bf_architecture.html | 2026-07-14 (Dev-63) | Library ✅ — GolfScorer node + publish-path note updated; ERD/D1 sections still current as of Dev-56, untouched — now further behind after Dev-67/70's schema additions, see Backlog |
...
| GET | `/feed` | None | Worker KV notification feed. Dev-69: also fires a non-blocking (`ctx.waitUntil`) prune on every call — card-tied announcement types auto-delete once their event's date has passed, on top of the existing 48hr blanket safety net. See Backlog & Known Issues. |
...
| **Cancelled Events** | ✅ Shipped Dev-84 | Commissioner marks a BF Series event cancelled → `canceled_events` flags-KV key set per event ID → card shows ❌ overlay wherever it renders (portal.html v4.2.0). Explicitly scoped to non-Gathering events only — Gatherings confirmed not to need this in the same session. |
...
| **Photo Upload Pause kill switch (Dev-60) is dead code** | ✅ Removed Dev-68 | `PHOTOS_UPLOAD_PAUSED` was set from the admin flags fetch and drove the toggle's own status display — but nothing anywhere actually checked it before allowing an upload. Never wired to a real check; the Meta Glasses 300-image overrun that originally motivated it (Dev-60) was already resolved by managing the upload folder directly, not by this switch. Rather than wire it up for a threat that no longer applies, removed entirely: the admin card, `PHOTOS_UPLOAD_PAUSED` state var, its flag load, its `devToggle()` branch, and its status-display refresh. The underlying `photos_upload_paused` KV key is now just an orphaned, harmless key — nothing reads or writes it anymore. |
| **`upload_attempts_log` (Dev-60) — confirmed dead, table dropped** | ✅ Removed Dev-68 | Built as a D1-backed daily rate-limit backstop during the same Dev-60 KV usage scare as the Photo Upload Pause switch above. Confirmed via full-file grep: zero references anywhere in `worker.js` — never written to, never read, never enforced. Brian ran `DROP TABLE IF EXISTS upload_attempts_log;` in the Cloudflare Console. |
| **Gathering Edit — no way to change Crew↔Open mode after creation** | ✅ Fixed Dev-68 | Real gap surfaced by Walli: he created a Crew-mode Gathering by mistake (wanted Open — "anyone can join") and discovered Edit had no mode toggle at all, only Create did. Fixed: Edit form now has the same "👥 Invite a Crew / 📢 Open to Members" toggle as Create, pre-selected from the Gathering's current mode, reusing the same crew-picker state/UI. Crew→Open just flips `fill_list_enabled`. Open→Crew (or reassigning a different crew) creates a new Crew via `POST /crews` if needed and PATCHes `crew_id` — required extending `PATCH /gatherings/:id`'s allowed fields (previously title/venue/event_time/size/gathering_type/description/tee_time_status/holes only). Newly-assigned crew gets a push, same copy as a fresh invite. **Follow-up same session:** the Edit path was missing the capacity-vs-crew-size hard block Create already has — a save went through with capacity 4 and only one person invited. Ported that validation over too: Crew mode + a capacity set now requires selected crew (+ host, if already registered Yes) to meet or exceed it, no override, matching Create exactly. Bonus fix: caught `\U0001F465` isn't valid JS escape syntax (silently rendered as literal text "U0001F465" instead of 👥) in both the Create and Edit crew-picker button — fixed both. |
| **Quick Rules card — USGA rules + BirdieFriends house rules** | ✅ Shipped Dev-68 | New 📖 Rules icon on the event card icon-row (alongside Photos/Score/Notes), two top-level tabs. **USGA tab:** Out of Bounds, Penalty Areas, Lost Ball, Net Double Bogey — content paraphrased from USGA's own player-facing Rules Hub, not the full rule book. OB/Lost Ball toggle between Standard (stroke-and-distance) and Quick-Play (USGA's own optional Local Rule E-5) since groups legitimately play both ways. Net Double Bogey section explains the max-hole-score-for-handicap-posting concept with two worked examples at the same Course Handicap (stroke received vs. not). **BirdieFriends tab:** house rules 1–6 reworded from the original 2024 BF Championship Format slide for clarity (points 7–8 excluded — those were scorekeeping steps tied to that year's specific app, not rules of play). Iterated twice on the Lie Adjustment rule's actual meaning (final: move up to one scorecard-length to the best lie within whatever type of ground the ball is already on — fairway/green/collar/rough/sand/hazard — never crossing into a different type; three worked examples). Rake-and-place allowance added to the Bunker Lies rule. Pure static content, no D1/Worker involvement. |
| **Layup Calculator (Yardage) card icon** | ✅ Shipped Dev-68 | 📏 icon on the event card, opens a simple two-number-input subtraction tool (distance to pin minus desired layup distance = how far to hit it). Deliberately no GPS/slope/wind/elevation and no distance measurement of its own — the player supplies both numbers — so it falls outside USGA Rule 4.3a's Distance Measuring Device restrictions entirely rather than needing to comply with them; same math as writing it on a scorecard by hand. |
| **GS Photo Organizer — briefly removed, restored same session** | ✅ Resolved Dev-68, both correct now | Brian asked to "clean up" the overlap between GS's native Photo Organizer (curation/chapters, feeds results.html at publish time) and My History (Dev-65+, always-current photo reference). First pass over-removed: deleted both the results.html Photos tab (the actual duplicate) *and* GS's own Photo Organizer tab (a real, reusable feasibility test for the eventual GLS-scale flagship-event photo workflow — not a duplicate of anything, just temporarily unused by one-day Series events). Brian caught the overreach immediately. Recovered cleanly: every GitHub push is a real commit, so the pre-removal version was pulled straight from git history (`raw.githubusercontent.com/.../<commit-sha>/source/BF_Golf_Scorer_8.html`) and the Photo Organizer restored byte-for-byte, while the results.html Photos tab removal was kept (that was the real duplicate). Net state: GS's Photo Organizer intact for future GLS/WallyCup-scale work; results.html no longer has a Photos tab; Live Panel's own photo capture untouched throughout. Worth remembering: nothing pushed to GitHub is ever really gone, a prior commit is always one `raw.githubusercontent.com/<sha>/...` fetch away — cheaper to recover than to rebuild. |
| **Live Panel + Card now coexist during a live round** | ✅ Shipped Dev-68 | Previously the live event's normal card was filtered out of Home's event list entirely while its Live Panel banner showed — meaning Rules/Notes/Yardage (card-only tools) were unreachable for the whole duration of a live round. Fixed: removed the exclusion filter, so the card renders alongside the Live Panel banner rather than being replaced by it. Live Panel keeps the live-only tools (Birdie Alert, CttP, the real competition-affecting Post-Round Scorecard); the card keeps everything else (Photos, Notes, Rules, Yardage, RSVP). **Caveat, per Brian's explicit design:** the card's own 🏌️ Score icon (the informal My History scorecard) now hides itself specifically while that event's Live Panel is open (`_livePanelOpen` true) — forcing scoring through the Live Panel's real Post-Round Scorecard during an active round, not the two side-by-side. Collapsed Live Panel or no live event → Score icon behaves normally. |
| **My History Photos/Score icon row** | ✅ Shipped Dev-69 | Same Photos/Score icons as the event card, added to My History's detail view so a player can add a photo or log a score for a round long after its card — and the `eventData`/`gatheringData` row behind it — has aged out of the loaded list. `historyEvtForSheets()` resolves the real evt object when still cached, or synthesizes a minimal one from the key alone when it's aged out (no venue/date, so par-autofill/weather-capture just no-op, same degraded-but-safe pattern already used elsewhere in that view). Caught and fixed before shipping: the three sheet modals (Photo/Score/Note) had no explicit `z-index`, so opening them from inside My History's higher-stacked detail view would have rendered them invisibly behind it — gave all three `z-index:9000`, same fix already applied to the photo lightbox/score-detail modal in Dev-67. Also fixed a real gap: `submitCardScore`/`submitCardNote`/`cardPhotoCameraPicked` never refreshed My History behind them on save, only the delete paths did. **Iterated same session on Brian's live feedback:** icons moved to the bottom of the card (after Notes, not just below the venue line); the Notes icon was dropped entirely — it opened a second, separate note-compose modal duplicating the full add/edit/delete thread My History already has inline, unlike Photos/Score which had no inline "add" path before this. |
| **Venue finder — Google Places results not showing, D1-only** | ✅ Fixed Dev-69 | Reported: Gathering panel's venue autocomplete only showed D1 "Your Courses," never live Google results. Two layered causes, found in sequence. First: `placesApiAvailable()`'s synchronous `google.maps.places.AutocompleteSuggestion` check is no longer reliable per Google's current docs — the class must be obtained via `await google.maps.importLibrary('places')`; fixed with a cached loader, plus a `window.gm_authFailure` handler and on-screen "Search Results: <reason>" diagnostics since Google auth failures don't throw normal catchable errors. That fix alone didn't resolve it — the diagnostic surfaced the real cause on the next report: `locationBias.radius` was 80000 (meters), over Google's hard 50000m cap for Places API (New), so every request was rejected outright with `"Invalid circle radius"`. Corrected to 50000. Confirmed fixed live by Brian. Diagnostics left in place permanently — cheap, invisible when working, saves a round trip if this class of failure recurs. |
| **Register All Crew — Gathering create toggle** | ✅ Shipped Dev-69 | New toggle on the create form's crew section (shown once ≥1 crew picked): registers the whole picked crew as Yes at creation time instead of individual RSVP, for a host who already knows everyone's playing. One `/registrations` POST per crew member (no bulk endpoint — not worth a Worker change for what's at most a dozen-ish parallel requests). Invite notification wording adjusts ("added you to... registered as playing" vs. the normal "invited you to"). Resets itself if the crew empties out or the host switches to Open mode. Scoped to Create only per the literal request ("gathering setup") — Edit-form parity not built, flagged as a possible quick follow-up. |
| **Score-entry auto-advance — reinstated for unknown-par venues** | ✅ Fixed Dev-69 | `csSetStroke` previously only auto-advanced to the next hole when real par was known for the venue; unknown-par venues stayed on the hole so the mark dots (only rendered for the active hole) were immediately visible. Per explicit request, unified — entering a score now always advances regardless of known par. Mark dots still reachable one tap back onto the hole number, just no longer automatic. |
| **Layup Calculator — compact single-row redesign** | ✅ Shipped Dev-69 | Reported: the stacked layout pushed the result below the on-course number keypad the moment either input was focused, forcing a scroll to see the answer. Redesigned to one row — Pin − Leave = Hit it — dropping the explanatory paragraph (labels now carry that meaning) and shortening result text to fit a compact cell. Modal is now handle + title/close row + one input row, no scroll possible. |
| **Announcement Feed auto-delete for card-tied types** | ✅ Shipped Dev-69 | `gathering_invite`/`gathering_open_invite`/`gathering_date_changed`/`gathering_cancelled`/`sub_promotion`/`new_event`/`event_reminder` now auto-delete from the KV feed once their underlying event's card would have expired, on top of the existing 48hr blanket safety net. The default/manual `broadcast` type (Brian's own hand-written announcements) is deliberately untouched — confirmed no general expiry rule wanted there, stays fully manual via Admin. Gathering-tied types resolve expiry via a live D1 lookup of `gatherings.event_time` by the `gathering_id` already present in their metadata (no client changes needed); live, not a send-time snapshot, so a rescheduled Gathering's old invite tracks the new date. The three Series-event types (currently dormant behind `OS_NOTIFY_*` flags) got `event_time` threaded through from `evt.dt` for when they're eventually enabled — no D1 row exists for the Worker to look those up by ID. `birdie`/`cttp` deliberately left on the 48hr-only rule — no event reference in their metadata today, flagged as an open scoping call. New shared `pruneAnnouncementFeed()` fires non-blocking via `ctx.waitUntil()` on every `GET /feed` (the real driver — every Home load) and after every send as a backstop; required widening the Worker's `fetch` signature to include `ctx`. **Process near-miss:** `source/worker.js` was pushed to GitHub without being presented for the Cloudflare paste in the same turn (a real miss against standing Rule 5, and the exact BL-19 risk already on this list) — caught when Brian asked directly, file presented and pasted, confirmed live. No lasting gap, but worth the repeat reminder. |
| **BSGC/BSCG venue-name typo — one scorecard fixed, then a 160-row bulk Jotform cleanup** | ✅ Shipped Dev-70 | Reported: a round's My History marks (bogey/birdie shapes) were missing despite real strokes saving correctly. Root cause: the event's Jotform "Event Location" was `BSCG`, a letter-transposed typo of `BSGC`, breaking the portal's exact-string venue match — par never resolved, and since marks are computed and stored at *save time* (not recomputed live), the blank marks were permanent for that scorecard. Fixed the one record directly, then confirmed via `Jotform:analyze_submissions` that the typo has been present "for years" — 25 rows in Request Event, 135 in Event Registration, 160 total. Built a temporary PIN-gated `/admin/fix-venue-typo` Worker route (dry-run + apply, dynamic qid lookup per form) since the Worker is the only piece of this stack with unrestricted outbound access to `api.jotform.com`. Hit Cloudflare's ~50-subrequest cap partway through — resolved by re-running the same idempotent call in batches until zero matches remained. Route removed after confirming all 160 fixed. |
| **"See Official Results" banner shown on non-Series events** | ✅ Fixed Dev-70 | `resultsClippingLink()`/`loadResultsClippingPodium()` were gated only on `evt.source !== 'gathering'`, so BF Weekend Times events (which have no results.html page) got the link too. Both now also require `formatBadge(evt.format) === 'BF Series'`, same classifier already used by Registration Tracker. |
| **Auto-Repeat Gatherings** | ✅ Shipped Dev-70, one round of live-verification needed | New checkbox on Gathering Create/Edit (crew-mode only): once a flagged Gathering's tee time passes, its successor auto-spawns a week later — same title/venue/format/capacity/crew — chaining until cancelled or unchecked, both of which fall out of the same `status='active' AND auto_repeat=1` spawn query for free. Triggered lazily via `ctx.waitUntil()` on every `GET /gatherings`, no Cron Trigger needed; race-safe via an atomic claim on `auto_repeat_spawned`; date math reuses the existing `localWallTimeToUTC()` DST-safe helper. Crew-mode-only because an Open Gathering's notify audience needs the live Jotform roster, which the Worker can't reach. New D1 columns `gatherings.auto_repeat`/`auto_repeat_spawned` (schema log Entry 25). **Real bug hit live:** the spawn query's `event_time < datetime('now')` never actually fired — comparing ISO-with-offset TEXT (`T` separator) against `datetime('now')`'s space-separated output as raw strings meant `T` (0x54) always sorted after a space (0x20), so the condition was false regardless of real elapsed time. Fixed by wrapping in `datetime()` so SQLite parses the offset first; reproduced against a real SQLite engine before shipping. **Near-miss caught before shipping:** while that bug was live, a host used the pre-existing manual Repeat button to create the real successor by hand — once the query fix was ready, the original card was still armed and would have spawned a *second*, duplicate Gathering the moment anyone next loaded Home. Caught and corrected via direct data check before the Worker paste. Permanent fix: `repeatGathering()` now hands the `auto_repeat` flag forward onto the new card and clears it from the source, so manual Repeat and the automatic engine coordinate instead of colliding. Not yet live-verified: an actual full auto-spawn cycle end to end (armed successor's own tee time hadn't passed as of session close). |
| **Gathering capacity — Yes could run unbounded past the stated size** | ✅ Shipped Dev-70 | Gatherings deliberately don't hard-lock at capacity like Series (Dev-45 design — Yes stays tappable past size with an advisory waitlist note), but `POST /registrations` had zero enforcement at all, confirmed as a real gap. Fixed: once existing Yes responses meet/exceed `gatherings.size`, a new Yes silently saves as Sub instead and the host gets notified by name/event. Both client RSVP paths now trust the server's actual saved status (not the tapped value) with a distinct "full — added as Sub" toast. Register All Crew (Dev-69) inherits this for free via the same endpoint — one known gap left as a follow-up: those requests fire in parallel, so a crew picked larger than capacity has a small race window before any commit lands. |
| **GS Groups ranking didn't match real Series standings — no 4-event qualification split** | ✅ Fixed Dev-70 | `grpBuildRankings()` pooled every scored player together and sorted purely by `seriesScore`, with no 4-event qualification gate — so a 3-event player with an unusually strong score could outrank genuine 4-event qualifiers, exactly the mismatch Brian caught against the published Series standings. Fixed by porting the real qualifier/non-qualifier split from `generateSeriesPage()` (same file) — qualifiers ranked 1..Q among themselves, non-qualifiers continue Q+1..N ranked only among each other. Also matched a subtlety: qualification count includes the synthetic retroactive-E1 event for NoHCP players, same as the real page, tracked via a separate `qualifyingEvts` field so the display event count stayed untouched. Not cosmetic — `overallRank` also drives the actual group-seeding sort, so this was misordering who got bucketed top/bottom-skill for balancing. Provisional ranks now render dimmed/parenthesized across all three render sites (cards, Players table, print card). |
| **GS unassigned pool required a manual button tap to stay rank-sorted** | ✅ Fixed Dev-70 | The existing "🏌 Sort Pool by Rank" button worked but had to be re-tapped after every Fetch or player returned to the pool. `grpRenderPool()` now always sorts by rank on render; the button stays, now redundant but harmless. |
| **GS local Jotform proxy — "more unavailable than available" at launch** | ✅ Fixed Dev-70 (one self-caught hotfix same session) | Diagnosed as a startup race, not a real outage — "Failed to fetch" is a raw browser network failure (no response at all), consistent with the browser firing its first request before `launch_golf_scorer.py` finishes binding its port; the Dev-64 threading fix never addressed a call landing before the server exists. Added a shared `fetchJotformProxy()` retry-with-backoff helper (5 attempts, 0/300/700/1200/2000ms, retries network failures + 5xx, not 4xx) across all 6 proxy call sites. **Process note:** the first push accidentally nested the helper inside `fetchCTPFromJotform()` instead of top-level scope, so every other call site couldn't see it — Node's syntax check passed (validates parse-ability, not scoping) so it shipped once before Brian's next message caught it; fixed within the same exchange, moved to genuine top-level, verified at column 0. |
| **GS HCP corrections silently reverting — three separate write paths, found in sequence** | ✅ Fixed Dev-70 (v8.53, v8.56, v8.57) | Reported twice, two different root causes. **First (v8.53):** `grpMergePlayers()`'s "always refresh from series history" step had no way to tell "auto-refresh" from "commissioner just corrected this," so any manual HCP edit (group-card or Players-table input) got silently overwritten on the next Fetch Registrants — e.g. right after a routine restart. Added an `hcpOverridden` flag, respected by the refresh guard, same pattern as the existing DiffHCP protection (Dev-64). **Second, the actual gap Brian was hitting (v8.56):** the GHIN copy/paste bulk tool writes `p.hcp` directly and was never routed through either patched function — GHIN-paste corrections (Brian's real workflow) kept reverting regardless of the first fix. Fixed by flagging that function's real-HCP-match branch too. **Third, found while auditing for the same class of bug (v8.57):** `grpUpdateHcp()` computed the correct `isNoHcp` value on every edit but never wrote it back to the player record — latent data-integrity bug, would have confused the same refresh guard. Worth the general lesson: the first fix covered the two write paths that seemed obvious and missed the one that was actually in daily use — grep for every direct write to a field before considering a "protect this" fix complete. |
| **GS NoHCP player stuck after an accidental HCP spinner click** | ✅ Fixed Dev-70 | A brand-new NoHCP player bumped to `0` via the HCP input's increment control had no way back. Two real bugs: the input's `min="0"` blocked the down-arrow from reaching blank (keyboard clear still worked, wasn't obvious); more critically, the Players-table tee cell's in-place DOM patch only ever restored the plain tee badge when *leaving* NoHCP, never restored the Green/Combo/Gold dropdown when *returning* to it — so a player converted away from NoHCP could never get a working tee-picker back. Fixed both directions of the patch, removed the `min="0"` floor (also legitimizes plus-handicap entry), and live-updates the input's placeholder to match current state immediately. |
| **Tom Kobal/Kobol — historical event filed under a misspelled name key** | ✅ Fixed Dev-70, one open question left for Brian | Surfaced as a false "first NoHCP event" baseline treatment for a player who'd already played once. His one real event (`2026 BFSeries#2`) was filed in GS's local `playerHistory` under `"Tom Kobol"`, a typo one character off the canonical Jotform roster spelling `"Tom Kobal"` — confirmed via `Jotform:analyze_submissions` against the Membership form. Exact-string lookups by the correctly-spelled roster name silently missed the misspelled historical key. Likely origin: GS's Players-tab manual "Add Player" field is free text with no roster autocomplete, unlike Fetch Registrants which always pulls the exact Jotform spelling. Fixed via Export JSON → precise rename of both the `playerHistory` key and the matching event's `scores` key (full-text swept to confirm no other occurrences) → corrected file delivered for Import — not yet confirmed imported/republished. **Separate, unresolved:** that historical record shows a real handicap (`17.9`) as of May 2026, but today's roster has him currently flagged NoHCP — a real status question for Brian to judge, not touched. **Offered, not built:** a datalist/autocomplete on the manual Add Player field tied to the roster, to prevent this class of typo going forward. |
