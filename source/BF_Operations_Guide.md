...
|-----------|---------|--------|
| Portal | v3.17.129 · 2026-07-30 | Production ✅ — Dev-69: My History Photos/Score icon row (bottom of card, Notes dropped as a duplicate of the inline compose already there), venue-finder Google Places fix (importLibrary switch + the actual root cause, a 50000m radius cap violation), Register All Crew host toggle on Gathering create, score-entry auto-advance reinstated for unknown-par venues, Layup Calculator compressed to a single row |
| GolfScorer | v8.50 · 2026-07-23 | Deployed ✅ — Dev-68: results.html Photos tab retired (My History is now the photo home for one-day Series events); native Photo Organizer tab kept intact for future GLS-scale flagship events (briefly removed then restored same session — see Known Issues) |
| Worker | 2026-07-30 (Dev-69) | Deployed ✅ — Announcement Feed auto-delete for card-tied notification types (`gathering_invite`/`gathering_open_invite`/`gathering_date_changed`/`gathering_cancelled`/`sub_promotion`/`new_event`/`event_reminder`), on top of the existing 48hr safety net. `fetch(request, env)` → `fetch(request, env, ctx)`. Manually pasted into Cloudflare dashboard and confirmed live by Brian. |
