...
|-----------|---------|--------|
| Portal | v3.11.4 · 2026-06-21 | Production ✅ |
...
- **During round (in-progress state):** Register/Unregister replaced by ⛳ "Round in progress · Xh Ym in · Tap the banner to enter scores" — **as of Dev-45 this card copy is stale/inaccurate for non-Live-Panel events** (Gatherings, BF Weekend Times, Wally/Cup/Scramble). The *gating logic* deciding who gets the actual Live Panel was corrected Dev-45 (was accidentally Series-only-by-coincidence via format string matching, now an explicit `hasLivePanelSupport()` chokepoint), but `buildEventCard()`'s in-progress text still unconditionally shows scoring-callout language for every format. The Tier 1 (generic, no scoring promise) vs. Tier 2 (capability-gated, real tap-target) card UI split — fully speced during the Dev-45 architecture debate, see Backlog — has **not** been built yet. This is the next logical piece of work: swap the hardcoded copy for a check against `hasLivePanelSupport(evt)`.
...
| **In-progress card copy still hardcoded to scoring language for every format** | 🔴 Next up | The gating *logic* (who actually gets the Live Panel) was fixed Dev-45 above — but `buildEventCard()`'s in-progress block still unconditionally renders "⛳ Round in progress · Tap the banner to enter scores" regardless of `hasLivePanelSupport(evt)`. So Gatherings/BF Weekend Times/Wally Cup events still show a false tap-promise in the card itself, even though the underlying Live Panel correctly won't open for them now. This is the **Tier 1 (generic, no scoring promise) vs. Tier 2 (capability-gated, real tap-target into the Live Panel)** card UI — fully speced during the Dev-45 architecture debate (see Session History): Tier 1 collapses to a minimal one-line "In-Progress" row for any in-progress event; Tier 2 swaps that row for a tap-target only where `hasLivePanelSupport(evt)` is true. Registration stays blocked on both tiers regardless. Two Live-mode concepts (Coordination Live — multi-group visibility; Integrity Live — auditable shared scoring/stakes, e.g. Tony Choy's side-game) were also captured as future considerations, independent of this fix. |
