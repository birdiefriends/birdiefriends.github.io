...
| **`portal_version.txt` disconnected from the in-app version display** | ✅ Fixed Dev-45, 🔲 worth hardening | Discovered live (Brian's screenshot, header still showing v3.11.3 after three deploys this session). The header/footer version string is hardcoded literally in two places in `portal.html` (`#portal-build-header`, `#portal-build`) — entirely separate from `source/portal_version.txt`, which is just a tracking doc fetched at session bootstrap, never read by the live page. Bumping the tracking file gives false confidence that the deployed version is correctly labeled. Fixed this session's drift (now both read v3.12.0). Not hardened: nothing prevents the same drift next session — a future pass could have the deploy step grep/sed both spots automatically, or have the page read `portal_version.txt` at runtime instead of hardcoding, so there's one source of truth instead of two that can silently diverge. |
