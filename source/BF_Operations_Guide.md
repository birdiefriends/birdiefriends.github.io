...
|-----------|---------|--------|
| Portal | v3.17.31 · 2026-07-13 | Production ✅ |
...
| Worker | 2026-06-18b | Deployed ✅ — `/deploy` accepts `source/` and `docs/` paths. No Worker changes Dev-63 — GS's new publish path reuses this existing route. |
...
| Launch_Golf_Scorer.bat / launch_golf_scorer.py | 2026-06-17 | Current ✅ — laptop-only, not in GitHub. Publish-relay token may now be unused, see Token Recovery above (Dev-63) |
...
| **`launch_golf_scorer.py` — "Failed to fetch"/needs-relaunch pattern** | ✅ Fixed Dev-64 | Root cause: the local launcher used `socketserver.TCPServer`, which handles exactly one request at a time. GS fires several proxy calls close together (GHIN Name map on every Groups-panel open/Apply per Dev-59's no-cache fix, plus portal events/registrations, sheets/netlify status, groupings, scorecard submissions) — while the server was busy on one, others queued behind it, and if any single call ever hung on the network the whole server froze for every caller until the process was killed and relaunched. Fixed by switching to a `socketserver.ThreadingMixIn` + `http.server.HTTPServer` combo (`daemon_threads=True`, `allow_reuse_address=True`) — each request now runs on its own thread, so one slow/stuck call can't block the rest; `allow_reuse_address` also reduces "port already in use" on quick relaunches. **Also fixed same session:** this file had never had a backup anywhere — laptop-only by design because it holds `JOTFORM_API_KEY` in plaintext. A sanitized copy (key blanked to `""`, real value never leaves the laptop) is now pushed to `source/launch_golf_scorer.py` as a reference/recovery backup — see Token Recovery in §3 above. Brian must paste the real key back into the **local runtime copy only**; the library copy stays blank on every future push. |
