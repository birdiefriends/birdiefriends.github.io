...
| **Photo Organizer needs a native home inside GS** | 🔲 Backlog, scoped, not started | All BF-Series post-round work (score entry, standings, payouts) already happens inside `BF_Golf_Scorer_8.html` — the Dev-57 Photo Capture curation panel (approve/reject/trophy/delete, grouped by event/section) lives only in the portal's commissioner admin panel, forcing a tool context-switch for one more post-round task. Backend is already fully shared and needs zero new work: `GET /photos?event=X&pin=7797`, `PATCH /photos/:id`, `DELETE /photos/:id` all exist today on the same Worker GS already calls (established this session via `grpPublish()`'s groupings sync). Scope is a UI port — the grouped-by-event/section rendering logic (`buildPhotoGroupedList()`/`renderTestPhotoThumb()` in `portal.html`) adapted into a new GS panel. Open decision for that session, not decided yet: build a native GS-styled panel (more consistent, more work) vs. iframe-embed the portal's existing admin panel (fast, less integrated) — mirrors the same embed pattern `groupings.html` already uses in the other direction. |
