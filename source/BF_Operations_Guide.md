...
| **Live Panel camera — only 1 of several players' photos reached the server at BFSeries#5** | 🟡 Two fixes shipped Dev-64, root cause NOT confirmed | Real live-event incident. Confirmed via `GET /photos?event=...` and `GET /debug/last-upload?pin=7797` that D1 has exactly one successful row (Jeff Rapp) — no trace of any other attempt, meaning the failure point was client-side, before any request reached the Worker (server-side failures already surface as toasts via `livePanelUploadCore()`'s try/catch). Mixed iOS/Android group affected, not isolated to one OS. Two fixes shipped on best available evidence: (1) removed the `android/allowCamera` bogus-MIME entry from the camera input's `accept` list — flagged fragile since Dev-57, never confirmed working on a real device, best-evidence hypothesis but **not provably the cause**; (2) fixed a real, separate, definitely-real bug found while investigating — `livePanelCameraPicked()`/`livePanelUploadPicked()` both silently `return`ed with zero user feedback when `input.files[0]` was empty (fires both on a legitimate cancel and on any other unexplained failure — no way to tell those apart client-side, so the fix is a quiet status-text update, not an alarming error toast). **First priority next live event:** watch whether Open Camera behaves consistently now across a mixed device group — this is the only real test available; nothing server-side can confirm or rule out the MIME-hack theory after the fact. |
