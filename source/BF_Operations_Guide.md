...
| **In-progress card behavior — architecture settled Dev-45, not yet implemented** | 🔲 Ready to implement | Originated Dev-44 (Gathering card showing misleading "tap the banner to enter scores"). Full architecture debate held Dev-45 — philosophy and placement now settled; this is the design to build against: **Three previously-conflated concepts, now separated:** (1) **Sunset/visibility** — purely list-membership, how long an event stays on Upcoming after tee time (the existing `format-series`→6hr / else→5hr window, `renderHome`). Unchanged. (2) **In-progress UI state** — a generic, format-agnostic time window (decoupled from sunset, not necessarily the same hours) during which registration/unregistration is blocked (correct on both tiers — can't sensibly register mid-round). (3) **Live capability** — whether the in-progress state is just informational or becomes a tap-target into real interactive features. **Two-tier model:** **Tier 1 (Default)** — applies to every in-progress event regardless of format. Collapses to a minimal one-line "In-Progress" row, no scoring language, no fake tap affordance — full card with badge risks reading as broken/dead-feature; a quiet collapsed row reads as intentional. Registration stays blocked. This slot is a placeholder for future lightweight, format-agnostic services (see Coordination Live below) — not guaranteed to have content for every event. **Tier 2 (Special/Live Panel)** — capability-gated; the in-progress row becomes a tap-target into a real interactive panel (today: CttP, scorecards, skins, Birdie Alerts — Series-only). **Gating axis reconsidered — not format, but problem-driven:** initially framed as "is this Series," then sharpened to group-size/coordination, then a second independent axis surfaced — see the two Live-mode triggers below. Format alone is no longer believed to be the right test for Tier 2 eligibility. **Live mode #1 — Coordination Live:** trigger = multiple groups who can't see each other (a visibility problem). A small 4-person Gathering doesn't need this — everyone's standing together — but a large multi-group Gathering has the identical cross-group-awareness problem a Series event does, regardless of format tag. Candidate features: informal messaging, notifications, "what's happening on a hole I can't see." Practical value for small/co-located groups is genuinely thin (open question whether Tier 1 has *any* content below the group-size threshold, or correctly has nothing). **Live mode #2 — Integrity Live:** trigger = a scoring/wagering structure whose math or stakes benefit from being shared and auditable in real time — independent of group size, applies even to a single 4some. Originating example (Tony Choy's private side-game: he privately tracks a side-bet on his own scorecard, announces who-owes-whom at the end, no one can verify the math or see it coming) — captured as a future custom-games/recommendation-engine consideration, **not** Dev-45/Gatherings scope, parked separately so it isn't lost. Candidate: live shared scorecard/tally for a side-game, panel-hosted, not Card-hosted either way. **Implementation note:** none of this is built yet — Dev-45 settled the model, not the code. Next session touching this should (a) decouple #2's window from sunset's, (b) build the Tier 1 collapsed row first since it unblocks Gathering cards immediately, (c) treat Tier 2 gating as a capability check to design later, not reuse the `format-series` boolean as-is. |
| **48hr-lock / 5th-player capacity logic is BF-Sat/Sun-specific but scoped to "has a capacity number," not to format** | 🔴 Blocking further Gathering Dev | Discovered Dev-44. `getCapacityStatus()`'s within-48hr lock + 5th-player/Schrödinger 2nd-tee-time logic triggers for *any* event with `evt.capacity` set, regardless of format — it was built for the standard Sat/Sun @ BSGC 8-player weekend game (which genuinely needs a 2nd tee time at 6 confirmed) and never formally scoped to just that case. Wally Cup/Scramble/Individual events with a capacity number currently run through the same logic whether or not it's actually true for them. Gatherings already got an explicit bypass (`evt.source === 'gathering'` skips the whole engine, Dev-44) — but that was a point fix for one format, not a designation of which formats this logic should even apply to. Needs an explicit tag (not "capacity is set") before this can be trusted as correct across formats. |
