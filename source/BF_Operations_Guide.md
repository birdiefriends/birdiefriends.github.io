...
| **In-progress card behavior is format-blind, content is wrong for non-Series formats** | 🔴 Blocking further Gathering Dev | Discovered Dev-44, reviewing a Gathering card. The post-tee-time "stay visible" *timing* is already generic (`format-series` → 6hr, everything else → 5hr, keyed off one binary) — Wally Cup, Scramble, Individual Score, and now Gatherings all already inherit it correctly. But the *content* shown in that window — "⛳ Round in progress · Tap the banner to enter scores" (`buildEventCard`, the `isInProgress` block) — fires for any format with no check, while `getLiveEvent()` (the actual scoring entry point) is hardcoded `format-series` only. So Wally/Cup/Scramble/Individual events have likely been showing this misleading scoring callout all along, unnoticed; Gatherings would too, and Gatherings have **no scoring at all** (spec §10). Needs a real per-format design pass — Brian wants cards to stay available during play regardless (communication + future in-play features matter more than hiding), but the messaging needs to be honest per format rather than assuming "in progress = there's a scoring banner." |
