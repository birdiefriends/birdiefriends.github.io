...
|-----------|---------|--------|
| Portal | v3.17.120 · 2026-07-23 | Production ✅ — Dev-68: venue weather coords (Whitetail/Moselem/Allentown Muni), Live Panel photo upload lock, Photo Upload Pause toggle removed, Quick Rules card (USGA + BirdieFriends house rules, two tabs), Layup Calculator, Gathering Edit mode switch (Crew↔Open) + capacity validation, Live Panel + Card now coexist during a live round (card Score icon hides while Live Panel is open) |
...
| Worker | 2026-07-23 (Dev-68) | Deployed ✅ — `PATCH /gatherings/:id` now accepts `fill_list_enabled`/`crew_id`, closing the gap where a Gathering's Crew-vs-Open mode couldn't be changed after creation |
