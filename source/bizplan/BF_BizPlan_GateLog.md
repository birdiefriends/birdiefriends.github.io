...
- **Scope boundary (settled): BF is explicitly not in the tee-time reservation business.** Every golf course already runs its own reservation system; BF has no intention of replacing or competing with those. This is a known gap, not a new discovery — flagged as both a barrier and an opportunity (see Cross-Gate Risks Register). The organizer must secure tee times outside BF, and that reservation is an external input *to* the "Schedule an Event" function, not something BF performs itself.
...
- **Tee-time reservation dependency:** the free "Schedule an Event" hook assumes tee times are already secured at a course — that reservation step happens entirely outside BF, course-by-course, on each course's own system. This is a known, longstanding gap (not newly discovered) and sits upstream of the hook rather than inside it. It's both a barrier (an extra off-platform step that can stall an organizer mid-funnel) and an opportunity (a future course-partnership or integration point nobody else may have solved well — see Cross-Gate Risks Register). Not resolved this session; flagged for Gate 5/6 thinking on whether and how to address it.
...
| **Architecture/dev-at-scale gap** | 2, 4, 5, 6 | Current platform is a working prototype that proves the concept (scoring, memorialization, live event flow — GLS-validated) but was not built for what Gates 1–2 now describe (multi-tenant architecture, payment processing, automated photo ingestion, recommendation engine, BF Pay compliance surface). **Update (BP-2):** Founder is starting a parallel dev experiment on multi-tenant architecture specifically — not hypothetical, validated by at least 3 BFs who have expressed direct interest. Previously infeasible under the old Jotform-based single-tenant data model; no longer blocked. Multi-tenant capability is core to the "Tee off, play great" tagline (see Vision) — the promise only holds if any BF/community can stand up their own instance without the founder hand-operating each one. Mitigation is in motion for this specific sub-gap; broader scale items (payments, automated ingestion, recommendation engine, BF Pay compliance) remain unaddressed. |
...
| **Tee-time reservation gap (external dependency)** | 1, 2, 5 | BF is explicitly not in the tee-time reservation business — every course runs its own system, and BF has no intent to replace those. But the "Schedule an Event" free hook implicitly assumes tee times are already secured, and that step happens entirely off-platform, course-by-course. Known longstanding gap, not new. **Barrier:** an extra off-platform step that can stall an organizer mid-funnel, outside BF's control. **Opportunity:** a future course-partnership or integration point (booking sync, course-side relationships) that could reduce that friction and may not be well-solved by competitors either — worth exploring as a differentiator rather than only a limitation. Not addressed this session; flagged for Gate 5/6. |
